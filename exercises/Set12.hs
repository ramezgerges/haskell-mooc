...
count x = foldr (\y z -> if x == y then z + 1 else z) 0
...
inBoth f g = foldr (\x z -> if x `elem` g then x : z else z) [] f
...
instance Foldable TwoList where
  ...
  foldr f z (TwoNode x y xs) =
    let nextFold = foldr f z xs
        firstFold = f y nextFold
     in f x firstFold

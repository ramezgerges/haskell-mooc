...
getName (MkPerson a p) = p

-- getAge returns the age of the person
getAge :: Person -> Int
getAge (MkPerson a p) = a

-- setName takes a person and returns a new person with the name changed
setName :: String -> Person -> Person
setName name (MkPerson a p) = MkPerson a name

-- setAge does likewise for age
setAge :: Int -> Person -> Person
setAge age (MkPerson a p) = MkPerson age p
...
data UpDown = IncreasingCounter Int | DecreasingCounter Int

-- zero is an increasing counter with value 0
zero :: UpDown
zero = IncreasingCounter 0

-- get returns the counter value
get :: UpDown -> Int
get (IncreasingCounter n) = n
get (DecreasingCounter n) = n

-- tick increases an increasing counter by one or decreases a
-- decreasing counter by one
tick :: UpDown -> UpDown
tick (IncreasingCounter n) = IncreasingCounter (n + 1)
tick (DecreasingCounter n) = DecreasingCounter (n - 1)

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle (IncreasingCounter n) = DecreasingCounter n
toggle (DecreasingCounter n) = IncreasingCounter n
...
prettyPrint = reverse . prettyPrint'
prettyPrint' End = ""
prettyPrint' (O rest) = '0' : prettyPrint' rest
prettyPrint' (I rest) = '1' : prettyPrint' rest

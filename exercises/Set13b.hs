...
path maze place1 place2 = evalState op []
  where
    op = do
      visit maze place1
      elem place2 <$> get
...
f1 thresh acc x = if acc + x > thresh then Nothing else Just (acc + x)
...
instance Monad Result where
  ...
  return = MkResult
  (>>=) (MkResult a) bind = bind a
  (>>=) NoResult bind = NoResult
  (>>=) (Failure str) bind = Failure str
...
instance Functor SL where
  -- implement fmap
  fmap g (SL f) = SL (\x -> (\case (a, i, ls) -> (g a, i, ls)) (f x))
...
instance Monad SL where
  ...
  (>>=) sl bind = SL out
    where
      out oldState =
        let (out1, newState, newLS) = runSL sl oldState
            newSL = bind out1
            (out2, newNewState, newnewLS) = runSL newSL newState
         in (out2, newNewState, newLS ++ newnewLS)

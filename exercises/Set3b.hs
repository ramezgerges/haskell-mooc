...
sums :: Int -> [Int]
sums i = reverse (sums' i)

sums' 1 = [1]
sums' i = case sums' (i - 1) of
    (a:xs) -> (a + i) : a : xs


reverse xs = reverse' xs []

reverse' (h:tail) acc = reverse' tail (h:acc)
reverse' [] acc = acc
...
indexDefault (h:tail) 0 def = h
indexDefault [] i def = def
indexDefault (h:tail) i def = indexDefault tail (i - 1) def
...
sumsOf :: [Int] -> [Int]
sumsOf xs = reverse (sumsOf' xs [])

sumsOf' [] acc = acc
sumsOf' (x:xs) [] = sumsOf' xs [x]
sumsOf' (x:xs) (accHead:accTail) = sumsOf' xs ((x+accHead):accHead:accTail)

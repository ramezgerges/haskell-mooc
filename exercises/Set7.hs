...
add a (Set []) = Set [a]
add a (Set (x:xs))
  | member a (Set (x:xs)) = Set (x:xs)
  | a <= x = Set (a:x:xs)
  | otherwise = let Set xs2 = add a (Set xs) in Set (x: xs2)
...
reverseNonEmpty = fromList . reverse . toList
...
passwordAllowed s (ContainsSome s2) = not $ null $ s `intersect` s2
passwordAllowed s (DoesNotContain s2) = not $ passwordAllowed s (ContainsSome s2)
passwordAllowed s (And p1 p2) = passwordAllowed s p1 && passwordAllowed s p2
passwordAllowed s (Or p1 p2) = passwordAllowed s p1 || passwordAllowed s p2
...
data Arithmetic = Const Integer | Addition Arithmetic Arithmetic | Multiplication Arithmetic Arithmetic
  deriving Show

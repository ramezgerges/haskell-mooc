...
treeMax Empty = 0
treeMax (Node root l r) = max (max (treeMax l) (treeMax r)) root
...
allValues condition (Node root l r) = condition root && allValues condition l && allValues condition r
...
mapTree f (Node root l r) = Node (f root) (mapTree f l) (mapTree f r)
...
cull val (Node root l r)
  | val == root = Empty
  | otherwise = Node root (cull val l) (cull val r)
...
isOrdered (Node root Empty Empty) = True
isOrdered (Node root Empty (Node rootRight l r)) = root <= rootRight && isOrdered (Node rootRight l r)
isOrdered (Node root (Node rootLeft l r) Empty) = root >= rootLeft && isOrdered (Node rootLeft l r)
isOrdered (Node root left right) = isOrdered (Node root Empty right) && isOrdered (Node root left Empty)
...
walk [] (Node root l r) = Just root
walk (StepL:xs) (Node root l r) = walk xs l
walk (StepR:xs) (Node root l r) = walk xs r
...
set [] val (Node root l r) = Node val l r
set (StepL:xs) val (Node root l r) = Node root (set xs val l) r
set (StepR:xs) val (Node root l r) = Node root l (set xs val r)

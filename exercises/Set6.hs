...
import Mooc.Todo
import Data.Char (toLower, GeneralCategory (FinalQuote))
...
instance (Price a, Price b) => Price (a,b) where
...
simplify (RationalNumber a b) = RationalNumber (a `div` g) (b `div` g) where g = gcd a b

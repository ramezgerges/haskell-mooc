...
-- statements xs ys = do x <- xs
--                       y <- ys
--                       [x ++ " is not " ++ y, x ++ " is " ++ y]
statements xs ys = concat $ (\x y -> [x ++ " is not " ++ y, x ++ " is " ++ y]) <$> xs <*> ys
...
calculator op n = maybeOp op <*> readMaybe n
  where maybeOp "negate" = pure negate
        maybeOp "double" = pure (*2)
        maybeOp _ = empty
...
validateAddress streetName streetNumber postCode = Address <$> checkStreetName streetName <*> checkStreetNumber streetNumber <*> checkPostCode postCode
  where checkStreetName s = check (length s <= 20) "Invalid street name" s
...
twoPersons :: Applicative f =>
  ...
  -> f [Person]
twoPersons name1 age1 employed1 name2 age2 employed2 = (\x y z x2 y2 z2 -> [Person x y z, Person x2 y2 z2]) <$> name1 <*> age1 <*> employed1 <*> name2 <*> age2 <*> employed2
...
boolOrInt s = validateBool s <|> validateInt s
  where validateInt s = check (isJust (readMaybe s :: Maybe Int)) "Not an Int" (Right $ read s)
        validateBool s = check (isJust (readMaybe s :: Maybe Bool)) "Not a Bool" (Left $ read s)
...
parseExpression x = if length (words x) == 3
                      then (\op arg1 arg2 -> op arg1 arg2) <$> parseOp(words x !! 1) <*> parseArg (head (words x)) <*> parseArg (words x !! 2)
                      else invalid $ "Invalid expression: "++x
  where parseVar x = check (length x == 1 && isLetter (head x)) ("Invalid variable: "++x) (Variable $ head x)
        parseNum x = check (isJust (readMaybe x :: Maybe Int)) ("Invalid number: "++x) (Number $ read x)
        parseArg x = parseNum x <|> parseVar x
        parseOp x = check (x == "+" || x == "-") ("Unknown operator: "++x) (if x == "+" then Plus else Minus)
...
myFmap f = myLiftA2 (\f a -> f a) (myPure f)
...
instance (Functor f, Functor g) => Functor (Both f g) where
  fmap h (Both a) = Both $ fmap (fmap h) a

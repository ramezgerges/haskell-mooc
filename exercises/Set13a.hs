...
split s =
  let ws = words s
   in if length ws < 2
        then Nothing
        else Just (head ws, concat (tail ws))
...
checkNumber' = foldr (\c z -> z && (not . isDigit) c) True
...
checkCapitals (for, sur) =
  if not (null for) && not (null sur) && isUpper (head for) && isUpper (head sur)
    then Just (for, sur)
    else Nothing
...
winner scores player1 player2 =
  do
    i1 <- lookup player1 scores
    i2 <- lookup player2 scores
    if i1 >= i2 then return player1 else return player2
...
selectSum xs [] = return 0
selectSum xs (i : is) = do
  when (length xs <= i || i < 0) Nothing
  let x = xs !! i
...
countAndLog pred (x : xs) = do
  when (pred x) (msg (show x))
  rest <- countAndLog pred xs
  return (rest + if pred x then 1 else 0)
...
balance accountName = BankOp op
  where
    op (Bank bank) = case Map.lookup accountName bank of
      Nothing -> (0, Bank bank)
      Just i -> (i, Bank bank)
...
-- rob from to = BankOp op
--   where op bank = let (bal, _) = runBankOp (balance from) bank in runBankOp (withdrawOp from bal +>> (depositOp to bal)) bank
...
count x = do
  xs <- get
  let ys = if any (\case (y, i) -> y == x) xs then map (count' x) xs else (x, 1) : xs
  put ys

count' y (x, i) = (x, i + (if x == y then 1 else 0))

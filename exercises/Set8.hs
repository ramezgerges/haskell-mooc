...
union (Shape f1) (Shape f2) = Shape (\x -> f1 x || f2 x)

cut :: Shape -> Shape -> Shape
cut (Shape f1) (Shape f2) = Shape (\x -> f1 x && not (f2 x))
------------------------------------------------------------------------------
...
paintSolid color (Shape sf) (Picture pf) = Picture p
  where p (Coord x y) | sf (Coord x y) = color
                      | otherwise = pf (Coord x y)
------------------------------------------------------------------------------
...
paint (Picture pat) (Shape sf) (Picture pf) = Picture p
  where p (Coord x y) | sf (Coord x y) = pat (Coord x y)
                      | otherwise = pf (Coord x y)
------------------------------------------------------------------------------
...
instance Transform Fill where
  apply (Fill c) (Picture f) = solid c
...
instance Transform Blur where
  apply Blur (Picture f) = Picture newF
    where newF (Coord x y) = Color ((r + r1 + r2 + r3 + r4) `div` 5) ((g + g1 + g2 + g3 + g4) `div` 5) ((b + b1 + b2 + b3 + b4) `div` 5) where
                                                            (Color r1 g1 b1) = f (Coord (x - 1) y)
                                                            (Color r2 g2 b2) = f (Coord (x + 1) y)
                                                            (Color r3 g3 b3) = f (Coord x (y - 1))
                                                            (Color r4 g4 b4) = f (Coord x (y + 1))
                                                            (Color r g b) = f (Coord x y)

...
rangeOf [] = 0
rangeOf [x] = 0
rangeOf (x:y:xs) = rangeOf' (min x y) (max x y) xs

rangeOf' min_ max_ [] = max_ - min_
rangeOf' min_ max_ (x:xs) = rangeOf' (min min_ x) (max max_ x) xs
...
longest = minimumBy compareList

compareList xs ys = if length xs /= length ys then compare (- length xs) (- length ys) else compare (head xs) (head ys)
...
incrementKey _ [] = []
incrementKey k ((k1,v1):xs) = (if k == k1 then (k1, v1 + 1) else (k1, v1)) : incrementKey k xs
...
winner scores player1 player2 = if score1 >= score2 then player1 else player2
    where score1 = lookupScore scores player1
          score2 = lookupScore scores player2

lookupScore scores player = case Map.lookup player scores of
    Nothing -> 0
    Just x -> x
...
freqs' [] m = m
freqs' (x:xs) m = case Map.lookup x m of
    Nothing -> freqs' xs (Map.insert x 1 m)
    Just a -> freqs' xs (Map.insert x (a+1) m)
...
transfer from to amount bank
    ...
    | otherwise = case Map.lookup from bank of
        Nothing -> bank
        Just a ->
            if a < amount then bank
            else case Map.lookup to bank of
                Nothing -> bank
                Just b -> Map.insert to (b + amount) (Map.insert from (a - amount) bank)
...
-- maxIndex arr = fst (sortBy (comparing snd) Data.Array.assocs arr ! 0)

maxIndex arr = fst (maximumBy (comparing snd) (Data.Array.assocs arr))

-- x = Data.Array.assocs (Array i e)
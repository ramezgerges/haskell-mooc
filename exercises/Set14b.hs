...
deposit conn txt n = execute conn depositQuery (txt, n)
...
balance conn txt = do
  r <- query conn balanceQuery [txt] :: IO [[Int]]
  return $ sum (head <$> r)
...
parseCommand (t : ts)
  | t == T.pack "balance" = if length ts == 1 then Just (Balance (head ts)) else Nothing
  | t == T.pack "deposit" = if length ts == 2 && isJust (parseInt $ ts !! 1) then Just (Deposit (head ts) (read $ T.unpack $ ts !! 1)) else Nothing
  | t == T.pack "withdraw" = if length ts == 2 && isJust (parseInt $ ts !! 1) then Just (Withdraw (head ts) (read $ T.unpack $ ts !! 1)) else Nothing
  | otherwise = Nothing
...
perform conn (Just command) = case command of
        Deposit txt n -> do
          ...
          return $ T.pack "OK"
        Withdraw txt n -> do
          deposit conn txt $ negate n
          return $ T.pack "OK"
        Balance txt -> do
          i <- balance conn txt
          return $ T.pack $ show i

perform cconn Nothing = return $ T.pack "ERROR"
...
server db request respond = do let path = pathInfo request
                               let commandMaybe = parseCommand path
                               resp <- perform db commandMaybe

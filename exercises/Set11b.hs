...
appendAll ref xs = modifyIORef ref (\r -> foldl (++) r xs)
...
hFetchLines h = do eof <- hIsEOF h
                   if eof then return []
                   else do content <- hGetContents h
...
hSelectLines h nums = do liness <- hFetchLines h
                         return $ map (\i -> liness !! (i - 1)) nums
...
interact' f state = do line <- getLine
                       let (cond, out, newSt) = f (line,state)
                        in do putStrLn out
                              if cond
                                then interact' f newSt
                                else return newSt
